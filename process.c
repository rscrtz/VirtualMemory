...
start_process (void *file_name_)
{
  ...
  hash_init (&t->sup_pages, page_hash, page_less, NULL);
...
process_exit (void)
{
  ...
  struct thread *child = NULL;
  struct list_elem *e, *prv;
  uint32_t *pd;

  hash_destroy (&cur->sup_pages, page_destructor);

  /* Close all opened files and free their pages */
  for (e = list_begin (&cur->openfiles); e != list_end (&cur->openfiles);)
    {
      prv = e;
      e = list_next (e);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      hash_insert (&t->sup_pages, &p->hash_elem);
